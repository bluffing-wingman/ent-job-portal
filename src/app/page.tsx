...
export default function Dashboard() {
  ...
  const recentJobs = db.prepare('SELECT * FROM jobs WHERE is_active = 1 ORDER BY created_at DESC LIMIT 5').all() as Job[]
  ...
  const quickLinks = [
    ...
    { href: '/hospitals', label: 'Hospitals', icon: Building2, color: 'bg-emerald-50 text-emerald-700 border-emerald-200', desc: `${totalHospitals} hospitals listed` },
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="card flex items-center gap-4">
          ...
          <div><p className="text-2xl font-bold text-gray-900">{totalActive}</p><p className="text-sm text-gray-500">Active Jobs</p></div>
        </div>
        <div className="card flex items-center gap-4">
          <div className="p-3 bg-green-50 rounded-lg"><TrendingUp className="h-6 w-6 text-green-600" /></div>
          <div><p className="text-2xl font-bold text-gray-900">{totalHospitals}</p><p className="text-sm text-gray-500">Hospitals</p></div>
        </div>
        <div className="card flex items-center gap-4">
          <div className="p-3 bg-purple-50 rounded-lg"><ClipboardList className="h-6 w-6 text-purple-600" /></div>
          <div><p className="text-2xl font-bold text-gray-900">{totalApplications}</p><p className="text-sm text-gray-500">Applications</p></div>
        </div>
        <div className="card flex items-center gap-4">
          <div className="p-3 bg-orange-50 rounded-lg"><Clock className="h-6 w-6 text-orange-600" /></div>
          <div><p className="text-2xl font-bold text-gray-900">{upcomingDeadlines}</p><p className="text-sm text-gray-500">Upcoming Deadlines</p></div>
        </div>
      </div>
